...
```
The mask has been correctly applied, as the dimensions of the object are not changed.

<<<<<<< HEAD
The number of genes studied diminishes in a 42 %. 

<!--- JOAN START --->
=======
>>>>>>> 1c293719ec1bcb14c2399df51ed49a98f0e5350c
# Normalization
...
As we can see in fig. 11 and 12, after filter those samples out, the distribution gains centrality and uniformity generating a more reliable data set. In addition, it seems that there is a better clustering by sample type than before.
...
The beforementioned increase of DE genes is explained due to the change of the model (correcting by an "expected-averaged-batch effect"), which fits better to the data, thus, enabling more genes to pass our significance threshold.
