...
After filtering, we obtain a dataset of 48 normal samples and 122 tumor (fig 3 and 4). We introduce it again in the prac.se dataset to obtain the final filtered version.
...
Seeing the graphs, we cannot establish differences between the tumoral/normal logCPM distribution.
...
#### Samplevial
```{r sva samplevial }
table(data.frame(TYPE=prac.se.pruned$type, TSS=samplevial))

# introduce the extracted data in the se obj
# samplevial <- substr(colnames(prac.se.pruned), 14, 16)
# colData(prac.se.pruned)$samplevial <- samplevial
# mod <- model.matrix(~ type + as.factor(samplevial), data = colData(prac.se.pruned))
# mod0 <- model.matrix(~ as.factor(samplevial), data = colData(prac.se.pruned))
# pValues <- f.pvalue(assays(prac.se.pruned)$logCPM, mod, mod0) 
# sum(p.adjust(pValues, method = "BH") < 0.01)
```
!!!!!!!!!!!!!!!
```{r pvales plot s, echo=FALSE, fig.align='center', fig.cap= 'p-values of the differential expression analysis using plate as null model.' }
hist(pValues, xlab= 'p-value')
```
