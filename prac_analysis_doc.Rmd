...
```{r exploring some variables, fig.align='center'}
sampleinfo.df <- as.data.frame(colData(prac.se))
par(mfrow=c(1,2))
ggplot(sampleinfo.df, aes(x=vital_status, fill =gleason_score)) + geom_bar()
ggplot(sampleinfo.df, aes(x=race, fill =gleason_score)) + geom_bar()
```
...
```{r gc content hist, fig.align='center'}
geneinfo.df <- as.data.frame(mcols(prac.se))
par(mfrow=c(1,2))
ggplot(geneinfo.df, aes(x=txlen)) + geom_histogram(bins = 50)
ggplot(geneinfo.df, aes(x=txgc)) + geom_histogram(bins = 50)
```
...
<!--- ADRI START --->
- By measuring surrogate variable analysis of plate and tss attributes, we observe a decrease in the quantity of DE genes. This implies that the attributes are not presenting non-biological variability in the study presented by the authors. 

- Wide studies like The Cancer Genome Atlas imply a really difficult process of filtering and assesing the data to obtain relevant and real results. Even with that treatment, we can always have a lot of variability making difficult the extraction of real or relevant conclusions. 

- There are present 7145 genes with a Differental Expression with a FDR of 1% (having as null model with a matrix of intercept term ~ 1).
