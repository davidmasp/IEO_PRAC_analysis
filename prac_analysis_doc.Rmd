...
dge.filtered_uniq <- dge_uniq[,(dge_uniq$samples$lib.size/1e6) > 50 ] # 50e06 as threshold value
...
plot(density(dge.filtered_uniq$samples$lib.size/1e6),main = "Density Plot of pruned samples by lib size")
...
After filtering, we obtain a dataset of 25 normal samples and 81 tumor. We introduce it again in the pracse dataset to obtain the final filtered version.

```{r final dataset}

pracse.final <- pracse.filt.unique[,rownames(dge.filtered_uniq$samples)]
```
