...
barplot((dge.filtered_uniq$sample$lib.size/1e06)[ord], las=1, ylab="Millions of reads",
        xlab="Samples", col=c("blue", "red")[(dge.filtered_uniq$sample$group[ord] == "tumor") + 1])
legend("topleft", c("tumor", "normal"), fill=c("red", "blue"), inset=0.01)
```
When filtering by non-paried data and also for library size (sequencing depth), we only have 1 normal out of 52 and 81 tumor samples out of 502. It is clearly not enough.
