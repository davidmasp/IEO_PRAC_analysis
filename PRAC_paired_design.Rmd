...
```{r InitialClustering, echo = FALSE, fig.height=10, fig.width=10, fig.cap="Fig. 7: Heatmap of Spearman distances without the disturbing samples."}
...
ggvolcano <- function(tt,FDR,hll = 3, hlr = 3,title = element_blank(), fc = 1){
  ...
  aes(logFC, B, label=symbol),angle = 25, vjust = -1) +
  guides(colour=FALSE) +
  theme(plot.title = element_text(size = rel(2)), panel.grid.minor = element_blank(),
...
```{r, echo=FALSE, eval = TRUE, dev= 'png', eval=FALSE}
...
```{r ClusteringGSVA, fig.height=15, fig.width=21 }
