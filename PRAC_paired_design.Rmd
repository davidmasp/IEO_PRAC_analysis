...
library(knitr)
opts_chunk$set(cache=T, fig.align='center', dpi=100, message=FALSE, dev=c('png','postscript'))
...
```{r loading packages and reading data, echo=TRUE, warning= FALSE, error=FALSE, message = TRUE, tidy=FALSE, results='hide'}
...
Initially, we observe the differental expression with a multiple linear regression test, having as a cutoff a FDR of 0.001. This highly restringent value have been used because of the large number of differential expressed genes in our data.
...
It is woth noting that the clear hit in the first 2 models (chr1-101) has a decrase in significance when we apply the SV correction. We believe that this decrease is impliying that it is an artifact instead of a hit. 
...
It can be observed that there are 6347 differential expresses genes (over and under represented). Now the procedure is repeated, conditioned by the child:
