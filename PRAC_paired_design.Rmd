...
Here there will be presented 3 different models plus de null model. The null model represents XXXX, then, 
...
volcanoplot(fit2, coef=2, highlight=7, fit3$genes$symbol, main="Model voom + patients", las=1, cex = 0.2)
par(mfrow=c(1,1))
volcanoplot(fit3, coef=2, highlight=7, fit3$genes$symbol, main="Final Model SV", las=1, cex = 0.2)
abline(-log10(FDRcutoff),0)
abline(v=1)
abline(v=-1)


ggvolcano <- function(tt,FDR,hl = 5,title = element_blank()){
  ggplot(tt, aes(logFC, B, 
  colour = ifelse(B>-log10(FDR) & (logFC < -1 |logFC > 1) ,"red","black" ))) +
  geom_point() + 
  ggtitle(title) + geom_vline(xintercept = c(1, -1), col = "red")+ 
  geom_hline(yintercept =-log10(FDR), 
             col = "red") + 
  geom_text(data= tt[order(tt$B,decreasing = TRUE),][(tt$logFC < -1 | tt$logFC > 1),][1:hl,], 
  aes(logFC, B, label=symbol),angle = 25, vjust = 0) +   
  guides(colour=FALSE) + 
  theme(plot.title = element_text(size = rel(3)))
}

ggvolcano(tt4,FDRcutoff,title = "Volcano Plot SV")
