...
par(mfrow=c(1,1))
volcanoplot(fit3, coef=2, highlight=7, fit3$genes$symbol, main="SV model", las=1, cex = 0.2, col = ifelse(fit3$  "red"))
abline(-log10(FDRcutoff),0)
abline(v=1)
abline(v=-1)


ggplot(tt4, aes(logFC, B, 
  colour = ifelse(B>-log10(FDRcutoff) & (logFC < -1 |logFC > 1) ,"red","black" ))) +
  geom_point() + 
  ggtitle("Volcano Plot SV") + geom_vline(xintercept = c(1, -1), col = "red")+ 
  geom_hline(yintercept =-log10(FDRcutoff), 
             col = "red") + 
  geom_text(data= tt4[order(tt4$B,decreasing = TRUE),][(tt4$logFC < -1 | tt4$logFC > 1),][1:5,], 
...
- ch2:10127
```{prova}
order(tt4$B,decreasing = TRUE),][(tt4$logFC < -1 | tt4$logFC > 1),][1:5,]
```
...
```{r}
DEgenes <- rownames(tt4)[tt$adj.P.Val < FDRcutoff]
length(DEgenes)
library(GOstats)
