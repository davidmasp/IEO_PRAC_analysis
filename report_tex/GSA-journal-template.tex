...
From the 11143 gene ontology (GO) terms for biological processes (BP) tested, only 21 presented significant results (p < 0.01). The top 5 results in odds ratio decreasing order are listed in Table \ref{tab:GOenrichment}.

The first result is related with processes that modulate the frequency, rate or extent of astrocyte differentiation (GO:0048710).

One consideration about the Gene Ontology is the variation of results with the parametic adjustment of the researcher. Pvalue cutoff and minimal gene sizes change the results of the functional enrichment analysis change considerably. This makes the assay subjective for each researcher, making the results quite bad. It is for this reason that our main functional analysis will be based in GSEA / GSVA results.
