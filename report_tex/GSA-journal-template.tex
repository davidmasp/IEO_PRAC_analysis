...
\caption{Heatmap of sample-to-sample distances. After filtering and pruning our data set the image show the difference of all genes expression in logCPM units. The Clustering have been performed using a Spearman method. We can observe clearly 2 groups corresponding to tumour and normal samples. 
}
...
As can be seen in the plot, there is an important number of genes that exceed the thresholds that were defined. Due that the number of loci would greatly exceed the amount of work, and in fact, it might be considered an article on its own, it was decided to reduce it to the 5 topmost scoring values.

Those 5 loci are chr1:225 which codifies for EPHA8, chr2:101 for SNORD89, chr3:385 SCN5A, chr11:11 for AP002884.2 (which seem to map to SDHD-006 and BCO2-009 subunits) and chrX:492 for CACNA1F.
}
