...
One of their important conclusions is that each molecular subtype present a significant different expression pattern. Considering that our analysis does not segregate for molecular subtype we already expect a big amount of variability that might be a caveat of our approach. However, it is possible that considering all the tumors we can get common insights from multiple subtypes of prostate cancer that can be also useful in future drug design studies. 

It is remarkable to note that the original paper did not report a proper differential expression analysis as the one presented in this paper. For the presented reason we believe a Differential Expression Analysis is needed to fully understand the nature of this data and to proper interpret a common insight from different molecular tumor sub-types. From our knowledge, this is the first study in this matter as other gene expression analysis of prostate cancer have been gene (or gene set) specific. 

 It is also important to consider that for further analysis it would be suitable to segregate the tumors in molecular sub-types before performing the analysis and that this step would increase the significance and value of the results. However, we were not able to distinguish between them as we did not have the corresponding data.
...
Detailed information about package versions can be found on the section "Session Info" of the Supplementary Materials (SM).
...
The data was used for this analysis is a variation of the above mentioned datasets, curated by PhD. Robert Castelo-Valdueza. In SM a link is attached in order to download it.
...
reprogramming of energy metabolism, generating genome instability,  evading immune destruction and promoting inflammation\citep{hanahan2011hallmarks}. In our analysis we had samples from several hallmarks coming from different analyses. We detected several proliferative signals like the HER2 pathway and the Insulin-like groth factor. We can also see the Interleukine-6 pathway also over-expressed in tumors that may be related with promoting inflamation in the tumor region. The EMT or activation of metastasis can be also observed in the deregulation of the cytoskeleton proteins and the disregulation of cholesterol homeostasis. 

Moreover, we can extract other conclusions. We have detected some calcium dependent pathways and proteins. 

calcium dependent cancer

sterols
 With Gene Set Expression Analysis we discovered the importance of SNAREs 
neuronal relation. 
